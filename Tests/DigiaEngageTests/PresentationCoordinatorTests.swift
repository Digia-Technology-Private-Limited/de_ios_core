...
struct PresentationCoordinatorTests {
    /// Watchdog timeouts default to "never" so a test that is not about them
    /// cannot be flaked by one.
    ...
    func anchorWatchdogFiresWhilePending() async throws {
        let coordinator = makeCoordinator(anchor: 0.02)
        let controller = coordinator.open(payload("g"), owner: "clevertap")
        coordinator.accept(controller, kind: .modal)
        coordinator.awaitAnchor(controller.trigger)
    ...
    @Test("G7 — anchorResolved disarms the anchor watchdog")
    func anchorWatchdogDisarms() async throws {
        ...
        coordinator.anchorResolved(controller.trigger)
        try await Task.sleep(nanoseconds: 120_000_000)
